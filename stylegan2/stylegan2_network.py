...
class ConstantInput(nn.Module):
    def __init__(self, channel, size=4):
        ...
        self.input = nn.Parameter(torch.randn(1, channel, 12, 4)) # 5 40 
...
class Generator(nn.Module):
    def __init__(
        ...
        layers.append(
            EqualLinear(
                style_dim, w_dim, lr_mul=lr_mlp, activation="fused_lrelu"
            ) 
        )
        for i in range(n_mlp-1):
            layers.append(
                EqualLinear(
                    w_dim, w_dim, lr_mul=lr_mlp, activation="fused_lrelu"
                )
            )
        ...
        self.log_size = int(math.log(size, 2)) # 6
